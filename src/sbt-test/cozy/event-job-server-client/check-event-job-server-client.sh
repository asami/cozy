...
sample_dir=/Users/asami/src/dev2026/cncf-samples/samples/05.b-event-job-server-client-lab
out_dir="$script_dir/out.d"
cml_file="$sample_dir/src/main/cozy/event.cml"
server_log="$out_dir/server.log"

cleanup_existing_servers() {
  pids=$(ps -ax | awk '/org\.goldenport\.cncf\.CncfMain --discover=classes server/ && /05\.b-event-job-server-client-lab|event-job-server-client\/out\.d/ {print $1}')
  if [ -n "$pids" ]; then
