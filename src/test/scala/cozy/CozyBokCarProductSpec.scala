 ...
 */
class CozyBokCarProductSpec extends AnyWordSpec with GivenWhenThen with SpecVocabulary {
    ...
    "register CAR product source packages" which {
      "discover .car-product packages and write publication registry metadata" in {
        _with_temp_dir("cozy-bok-car-product") { dir =>
          Given("a BoK source tree with a CAR product package and an external CAR project reference")
          val externalproject = dir.resolve("external/nict-knowledgehub")
          _write(externalproject.resolve("build.sbt"), "ThisBuild / version := \"0.1.0\"\n")
          _write(externalproject.resolve("src/main/cozy/nict-knowledgehub.cml"),
            """# ENTITY
              |
              |## KnowledgeItem
              |
          ...
          _write(dir.resolve("conf/cozy/config.yaml"),
            s"""bok:
               ...
               |""".stripMargin)
          val pkg = dir.resolve("src/main/doxsite/concept/nict-knowledgehub.car-product")
          _write(pkg.resolve("index.dox"), "NictKnowledgeHub\n================\n")
          _write(pkg.resolve("product.yaml"),
            """product:
              |  type: car
              ...
              |""".stripMargin)
          val config = CozyBok.PublicationConfig.create("publish-car-products", List(dir.toString))

          When("Cozy registers CAR product publication metadata")
          val results = CozyBok.publishCarProducts(config)

          Then("the publication registry is written without creating generated files inside the source package")
          results.map(_.product.name) shouldBe Vector("nict-knowledgehub")
          dir.resolve("src/main/publication/nict-knowledgehub.json") should beRegularFile
          val bundle = _read(dir.resolve("src/main/publication/nict-knowledgehub.json"))
          bundle should include ("metadata/products/car/nict-knowledgehub/metadata.json")
          bundle should include ("metadata/products/car/nict-knowledgehub/0.1.0/manifest.json")
          bundle should include ("metadata/artifacts/repository/nict-knowledgehub.json")
          bundle should include ("products/nict-knowledgehub")
          bundle should include ("\"status\" : \"missing\"")
          bundle should include ("CAR artifact is not registered in warehouse")
          bundle should include ("Run cozy publish-car <nict-knowledgehub>")
          bundle should not include (externalproject.toAbsolutePath.normalize().toString)
      ...
      "record published artifact metadata when the CAR already exists in the warehouse" in {
        _with_temp_dir("cozy-bok-car-product-artifact") { dir =>
          Given("a CAR product package and a pre-existing warehouse CAR artifact")
          val externalproject = dir.resolve("external/nict-knowledgehub")
          _write(externalproject.resolve("build.sbt"), "ThisBuild / version := \"0.1.0\"\n")
          _write(dir.resolve("conf/cozy/config.yaml"),
            s"""bok:
          ...
          _write(pkg.resolve("product.yaml"),
            """product:
              ...
              |  path: products/nict-knowledgehub
              |""".stripMargin)
          _write(dir.resolve("warehouse/repository/car/nict-knowledgehub/0.1.0/nict-knowledgehub-0.1.0.car"), "car-body")
          val config = CozyBok.PublicationConfig.create("publish-car-products", List(dir.toString))
          ...
          bundle should include ("repository/car/nict-knowledgehub/0.1.0/nict-knowledgehub-0.1.0.car")
        }
      }

      "derive CAR version and artifact references from the warehouse catalog when descriptor version is omitted" in {
        _with_temp_dir("cozy-bok-car-product-catalog") { dir =>
          Given("a CAR product package that relies on the warehouse CAR catalog")
          val externalproject = dir.resolve("external/nict-knowledgehub")
          _write(externalproject.resolve("build.sbt"), "ThisBuild / version := \"0.2.0\"\n")
          _write(dir.resolve("conf/cozy/config.yaml"),
            s"""bok:
               |  projects:
               |    nict-knowledgehub:
               |      path: ${externalproject.toString}
               |""".stripMargin)
          _write(dir.resolve("warehouse/repository/car/nict-knowledgehub/0.2.0/nict-knowledgehub-0.2.0.car"), "catalog-car-body")
          _write(dir.resolve("warehouse/repository/catalog/car/nict-knowledgehub.yaml"),
            """schemaVersion: 1
              |kind: car
          ...
          Then("the product version and artifact metadata are sourced from the warehouse catalog")
          results.head.product.version shouldBe "0.2.0"
          results.head.product.versionsource shouldBe "warehouse-catalog"
          results.head.artifactexists shouldBe true
          val bundle = _read(dir.resolve("src/main/publication/nict-knowledgehub.json"))
          bundle should include ("\"version\" : \"0.2.0\"")
          bundle should include ("\"versionSource\" : \"warehouse-catalog\"")
          bundle should include ("repository/catalog/car/nict-knowledgehub.yaml")
          bundle should include ("repository/car/nict-knowledgehub/0.2.0/nict-knowledgehub-0.2.0.car")
        }
      }

      "resolve external project paths from local .cozy overrides" in {
        _with_temp_dir("cozy-bok-car-product-local-config") { dir =>
          Given("public config points to a non-local reference and .cozy contains the local sensitive path")
          ...
          Then("the local sensitive .cozy path overrides the public config path")
          results.head.product.projectpath.map(_.toString) shouldBe Some(externalproject.toAbsolutePath.normalize().toString)
          val bundle = _read(dir.resolve("src/main/publication/nict-knowledgehub.json"))
          bundle should not include (externalproject.toAbsolutePath.normalize().toString)
      ...
      "reject generated .car-product.d work directories" in {
        _with_temp_dir("cozy-bok-car-product-workdir") { dir =>
          Given("a BoK source tree containing a reserved CAR product work directory")
          Files.createDirectories(dir.resolve("src/main/doxsite/concept/generated.car-product.d"))
          val config = CozyBok.PublicationConfig.create("publish-car-products", List(dir.toString))
    ...
    "integrate CAR products with BoK publication commands" which {
      "update-publication handles CAR products without invoking CAR artifact publishing" in {
        _with_temp_dir("cozy-bok-car-product-update") { dir =>
          Given("a BoK source tree with one CAR product and no warehouse artifact")
          val externalproject = dir.resolve("external/nict-knowledgehub")
          _write(externalproject.resolve("build.sbt"), "ThisBuild / version := \"0.1.0\"\n")
          _write(dir.resolve("conf/cozy/config.yaml"),
            s"""bok:
          ...
          _write(pkg.resolve("product.yaml"),
            """product:
              ...
              |  path: products/nict-knowledgehub
              |""".stripMargin)
          val config = CozyBok.PublicationConfig.create("update-publication", List(dir.toString))

          When("Cozy updates the BoK publication registry")
          val updated = CozyBok.updatePublication(config, RecordingVoicevoxClient, NoopVideoRunner)

          Then("only publication metadata is written; no CAR artifact is generated")
          updated should contain ("nict-knowledgehub")
          dir.resolve("src/main/publication/nict-knowledgehub.json") should beRegularFile
          dir.resolve("warehouse/repository/car/nict-knowledgehub/0.1.0/nict-knowledgehub-0.1.0.car") shouldNot existPath
        }
      }
        ...
        _with_temp_dir("cozy-bok-car-product-dry-run") { dir =>
          ...
          _write(pkg.resolve("product.yaml"),
            """product:
              ...
              |  path: products/nict-knowledgehub
              |""".stripMargin)
          val config = CozyBok.PublicationConfig.create("publish", List(dir.toString, "--dry-run"))

          When("Cozy prints the one-stop publish plan")
          val out = _capture {
            CozyBok.publish(config, new RecordingRunner, RecordingVoicevoxClient, NoopVideoRunner)
          }

          Then("the plan and manifest include CAR product package counts without side effects")
          ...
          parser.parse(manifest).fold(throw _, identity).hcursor.downField("dryRun").as[Boolean].fold(throw _, identity) shouldBe true
        }
      }
      ...
      "publish-car writes CML and model-metadata sidecars into the warehouse catalog" in {
        _with_temp_dir("cozy-bok-car-product-publish-car-sidecars") { dir =>
          Given("a CAR project with a CML source file and a prebuilt CAR archive")
          val projectdir = dir.resolve("project")
          val warehouse = dir.resolve("warehouse")
          val car = _write(dir.resolve("input/nict-knowledgehub.car"), "car-body")
          _write(projectdir.resolve("project.yaml"),
            """project:
              |  name: nict-knowledgehub
              |packaging:
              |  car:
              ...
              |        minimum: 0.4.8
              |""".stripMargin)
          _write(projectdir.resolve("src/main/cozy/nict-knowledgehub.cml"),
            """# ENTITY
              |
              |## KnowledgeItem
              |
          ...
          Then("the warehouse CAR catalog contains the CML source and machine-readable model metadata sidecars")
          ...
          catalogdir.resolve("nict-knowledgehub.model-metadata.json") should beRegularFile
          catalogdir.resolve("nict-knowledgehub.model-metadata.yaml") should beRegularFile
          val metadata = _read(catalogdir.resolve("nict-knowledgehub.model-metadata.json"))
          metadata should include ("cozy.cml.model-metadata.v1")
          metadata should include ("\"path\" : \"src/main/cozy/nict-knowledgehub.cml\"")
          metadata should not include (projectdir.toAbsolutePath.normalize().toString)
      ...
      "prefer warehouse model-metadata over direct CML scan for CAR product registration" in {
        _with_temp_dir("cozy-bok-car-product-model-metadata-priority") { dir =>
          Given("a BoK CAR product whose warehouse catalog has generated model metadata")
          val externalproject = dir.resolve("external/nict-knowledgehub")
          _write(externalproject.resolve("build.sbt"), "ThisBuild / version := \"0.1.0\"\n")
          _write(externalproject.resolve("src/main/cozy/nict-knowledgehub.cml"),
            """# ENTITY
          ...
          _write(dir.resolve("conf/cozy/config.yaml"),
            s"""bok:
               ...
               |      path: ${externalproject.toString}
               |""".stripMargin)
          _write(dir.resolve("warehouse/repository/catalog/car/nict-knowledgehub.model-metadata.json"),
            """{
              |  "schema": "cozy.cml.model-metadata.v1",
              |  "source": {
              |    "path": "warehouse-sidecar.cml",
              |    "sha256": "sidecar",
              ...
              |        "summary": "Warehouse metadata summary.",
              |        "description": "Warehouse metadata description."
              |      },
              |      "narrative": "Warehouse metadata narrative."
              |    }
              |  ]
              |}
              |""".stripMargin)
          ...
          Then("the registry uses the warehouse model-metadata and does not fall back to direct CML scan")
          val bundle = _read(dir.resolve("src/main/publication/nict-knowledgehub.json"))
          bundle should include ("warehouse-model-metadata")
          bundle should include ("CatalogEntity")
          bundle should include ("Warehouse metadata summary.")
          bundle should include ("Warehouse metadata narrative.")
      ...
      "help lists CAR product publication commands" in {
        ...
        bokhelp should include ("CAR artifact publishing remains the responsibility of cozy publish-car")
      }

      "build renders CAR product article pages at their publication path" in {
        _with_temp_dir("cozy-bok-car-product-build") { dir =>
          Given("a BoK source tree with a CAR product package and publication path")
          val externalproject = dir.resolve("external/nict-knowledgehub")
          _write(externalproject.resolve("build.sbt"), "ThisBuild / version := \"0.1.0\"\n")
          _write(externalproject.resolve("src/main/cozy/nict-knowledgehub.cml"),
            """# ENTITY
          ...
          Then("the CAR product article is rendered as a normal BoK page without generating CAR artifacts")
          val page = _read(dir.resolve("website.d/textus/components/nict-knowledgehub/index.html"))
          ...
          val termhub = _read(dir.resolve("website.d/glossary/cml/knowledge-item.html"))
  ...
  private object RecordingVoicevoxClient extends cozy.video.CozyVideo.VoicevoxClient {
    def speakers(baseurl: String): io.circe.Json =
      io.circe.Json.arr()
    def audioQuery(baseurl: String, text: String, speakerid: Int): io.circe.Json =
      io.circe.Json.obj()
    def synthesis(baseurl: String, speakerid: Int, audioquery: io.circe.Json): Array[Byte] =
      Array.emptyByteArray
  }

  private object NoopVideoRunner extends cozy.video.CozyVideo.VideoProcessRunner {
    def run(command: Vector[String], cwd: Path): cozy.video.CozyVideo.VideoCommandResult =
