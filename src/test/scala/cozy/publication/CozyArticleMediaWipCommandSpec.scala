    ...
    "parse the frozen strict grammar" which {
      "accept separated and equals forms and reject every ambiguous option edge" in {
        ...
        _with_fixture("grammar") { fixture =>
          ...
          val relativeconfig = _failure(CozyArticleMediaWipCommand.Config.create(List(descriptor, "--publication", publication, "--website", website, "--site-root", siteroot, "--site-config", "relative-site.conf")))
          ...
          relativeconfig.getMessage should include("--site-config path must be absolute")
