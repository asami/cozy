...
class CozyBokTermHubSpec
  ...
  private def _terms_json: String =
    """{
      ...
      |        "actors": ["architecture:knowledge-owner"],
      |        "roles": ["architecture:reviewer"],
      |        "participants": ["architecture:knowledge-owner", "architecture:reviewer"],
      |        "scenarios": ["scenario:review"],
      |        "evidence": ["bib:design-patterns"],
      |        "cml_event": "knowledge.published",
      |        "cml_component": "knowledgehub",
      |        "cml_statemachine": "KnowledgeItemLifecycle"
      |      },
