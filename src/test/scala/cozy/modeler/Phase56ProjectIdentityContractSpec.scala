...
final class Phase56ProjectIdentityContractSpec
  ...
  "E5 canonical project identity authoring" should {
    "E5 load one canonical-only project.yaml as project metadata" must _e5 {
    ...
    "E5 route the same canonical config through production scaffold generation" must _e5 {
      "E5 emits canonical identity through the scaffold chain" in {
        ...
        _with_temp_dir("cozy-phase56-cid01-e5-scaffold") { directory =>
          ...
          Then("E5 the generated project identity and legacy-field absence match the contract")
          pendingUntilFixed {
