...
class Cozy(
  ...
  def executeDirect(args: Array[String]): Unit = {
    if (!_execute_car_sbt_project(args) && !_execute_publish_car(args) && !_execute_publish_project(args) && !_execute_distribute_samples(args) && !_execute_index_warehouse(args) && !_execute_sbt_bridge(args) && !_execute_package_archive(args))
...
object Cozy {
  ...
  private[cozy] val helpText: String =
    """Usage:
      ...
      |      Publish a CAR archive and CAR catalog to a warehouse. Maven metadata is generated by a later publish step.
      |
