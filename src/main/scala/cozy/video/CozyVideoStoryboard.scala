...
private[cozy] trait CozyVideoStoryboard {
  ...
  def storyboardIdentity(storyboard: Storyboard): String =
    "sha256:" + _sha256_hex(canonicalStoryboardJson(storyboard).getBytes(StandardCharsets.UTF_8))
  ...
  private def _storyboard_json(storyboard: Storyboard): Json =
    Json.obj(
  ...
  private def _scene_json(scene: StoryboardScene): Json =
    Json.obj(
  ...
  private def _production_insert_json(insert: StoryboardProductionInsert): Json =
    Json.obj(
  ...
  private def _pronunciation_note_json(note: StoryboardPronunciationNote): Json =
    Json.obj(
  ...
  private def _json_string(value: String): String = Json.fromString(value).noSpaces

  private def _decimal_text(value: BigDecimal): String = _normalized_decimal(value).bigDecimal.toPlainString

  private def _normalized_decimal(value: BigDecimal): BigDecimal = {
  ...
  private def _is_integer(value: BigDecimal): Boolean =
