...
private[cozy] object CozyMediaPresentation {
  ...
  object ResourceConfig {
    implicit val decoder: Decoder[ResourceConfig] = (c: HCursor) =>
      for {
        _ <- _require_exact_keys(c, Set("profile", "slideImages", "montage", "rendererManifest", "reviewManifest", "reviewState", "articlePdf", "infographic"), "presentation resource")
      ...
      } yield ResourceConfig(profile, slideimages, montage, renderermanifest, reviewmanifest, reviewstate, articlepdf, infographic)
  }
  ...
  private def _validate_receipt(descriptor: CozyMedia.Descriptor, resource: CozyMedia.Resource, profile: ProfileConfig, root: Path): Unit = {
    ...
    if (!receipt.inputs.exists(input => input.role == "slide-ir" && input.path == source && input.normalization == "structured-document")) _invalid(s"Presentation receipt requires structured-document slide IR input: ${resource.id}")
