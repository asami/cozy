...
private[cozy] object CozyDocumentProjectEvidence {
  ...
  private[cozy] def captureNativeInputs(
    ...
    CozyDocumentProjectNativeEvidence.captureNativeInputs(project, descriptor, operation)
  ...
  private[cozy] def closeNativeExecution(
    ...
    CozyDocumentProjectNativeEvidence.closeNativeExecution(project, descriptor, operation, inputs, result)
  ...
  def snapshot(project: Path, descriptor: CozyDocumentProject.Descriptor): Snapshot = {
    ...
    val nativeoperations = CozyDocumentProjectExecutability.nativeOperationStates(project, descriptor, resolved, sources, products, attempts, presentationstate)
  ...
  private[cozy] def nativeOperationStateLines(value: Snapshot): Vector[String] =
    CozyDocumentProjectExecutability.nativeOperationStateLines(value)
  ...
  private[cozy] def planNativeOperationStateLines(
    ...
    val presentationstate = CozyDocumentProjectPresentationSemanticsState.derive(project, descriptor)
    val products = _products(project, descriptor, resolved, sources, sidecar, attempts, presentationstate)
    CozyDocumentProjectExecutability.nativeOperationStates(project, descriptor, resolved, sources, products, attempts, presentationstate).map(CozyDocumentProjectExecutability.nativeOperationStateLine)
  }

  def stateYaml(project: Path, descriptor: CozyDocumentProject.Descriptor): String =
    CozyDocumentProjectExecutability.stateYaml(project, descriptor, snapshot(project, descriptor))
  ...
  def stateYaml(
    ...
    CozyDocumentProjectExecutability.stateYaml(project, descriptor, value)
  ...
  private def _generated_review_status(
    ...
    if (!Files.exists(receiptpath, LinkOption.NOFOLLOW_LINKS)) {
      ...
      val outputcurrent = CozyDocumentProjectNativeEvidence.identityCurrent(project, receipt.output)
      val inputscurrent = receipt.inputs.forall(identity => CozyDocumentProjectNativeEvidence.identityCurrent(project, identity))
  ...
  private def _article_review_status(
    ...
    if (native.isEmpty) {
      ...
      if (accepted.exists(CozyDocumentProjectNativeEvidence.nativeAttemptCurrent(project, _))) {
        EvidenceStatus("current", None)
  ...
  private def _attempt(project: Path, descriptor: CozyDocumentProject.Descriptor, path: Path): Attempt = {
    ...
    _string(fields, "schema", "Document Project retained attempt") match {
      ...
      case `_attempt_v2_schema` => CozyDocumentProjectNativeEvidence.parseRetainedAttempt(project, descriptor, path)
