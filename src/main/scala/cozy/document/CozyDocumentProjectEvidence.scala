...
private[cozy] object CozyDocumentProjectEvidence {
  ...
  final case class AttemptOutput(identity: String, path: String, mediaType: String, sha256: String)
  final case class Attempt(
    ...
    outputs: Vector[AttemptOutput]
  )
  sealed trait NativeClosure {
    def attempt: Attempt
    def diagnostics: Vector[String]
  }
  final case class NativeAcceptedClosure(attempt: Attempt, diagnostics: Vector[String]) extends NativeClosure
  final case class NativeFailedClosure(attempt: Attempt, diagnostics: Vector[String]) extends NativeClosure
  ...
  private val _attempt_v1_schema = "cozy.document-operation-attempt.v1"
  ...
  private[cozy] def captureNativeInputs(
    project: Path,
    descriptor: CozyDocumentProject.Descriptor,
  ...
  private[cozy] def closeNativeExecution(
    project: Path,
    descriptor: CozyDocumentProject.Descriptor,
    operation: CozyDocumentWorkflow.LogicalOperation,
    inputs: Vector[FileIdentity],
  ...
  private def _validate_native_executed(
    ...
    if (inputs != captureNativeInputs(project, descriptor, operation) || !inputs.forall(identity => _identity_current(project, identity)))
  ...
  private def _append_v2_attempt(
    ...
    try {
      ...
      Attempt(
        ...
        operation.id,
        outcome,
        if (outcome == "accepted" || outcome == "failed") operation.produces else Vector.empty,
        inputs,
  ...
  private def _products(
    ...
    val initial = resolved.workProducts.map { value =>
      ...
      if (value.selection == CozyDocumentWorkflow.WorkProductSelection.ProfileDisabled) {
        ...
        product.id -> _article_review_status(project, descriptor, sourcepaths, attempts)
  ...
  private def _article_review_status(
    project: Path,
    descriptor: CozyDocumentProject.Descriptor,
    sourcepaths: Set[String],
    attempts: Vector[Attempt]
  ): EvidenceStatus = {
    val native = attempts.filter(attempt => attempt.schema == _attempt_v2_schema && attempt.operation == "article.render-review")
    if (native.isEmpty) {
      _generated_review_status(project, descriptor, sourcepaths, "article")
    } else {
      val accepted = native.filter(_.outcome == "accepted")
      if (accepted.exists(_native_attempt_current(project, _))) {
        EvidenceStatus("current", None)
      } else if (accepted.nonEmpty) {
        EvidenceStatus("stale", Some("accepted native evidence input or declared output is stale"))
      } else {
        EvidenceStatus("missing", Some("no accepted native evidence is retained"))
      }
    }
  }

  private def _native_attempt_current(project: Path, attempt: Attempt): Boolean =
    attempt.inputs.forall(identity => _identity_current(project, identity)) &&
      attempt.outputs.forall { output =>
        _identity_current(project, FileIdentity(output.path, output.sha256))
      }
  ...
  private def _attempt(project: Path, descriptor: CozyDocumentProject.Descriptor, path: Path): Attempt = {
    ...
    _string(fields, "schema", "Document Project retained attempt") match {
      case `_attempt_v1_schema` => _attempt_v1(project, descriptor, path)
      case `_attempt_v2_schema` => _attempt_v2(project, descriptor, path)
      case _ => _invalid("Document Project retained attempt schema is unsupported")
    }
  }

  private def _attempt_v1(project: Path, descriptor: CozyDocumentProject.Descriptor, path: Path): Attempt = {
    ...
    if (fields.keySet != expected || _string(fields, "schema", "Document Project retained attempt") != _attempt_v1_schema) {
    ...
    Attempt(FileIdentity(CozyDocumentProject._project_relative(project, path), _sha256(path)), _attempt_v1_schema, operation.id, outcome, products, Vector.empty, Vector.empty)
  }

  private def _attempt_v2(project: Path, descriptor: CozyDocumentProject.Descriptor, path: Path): Attempt = {
    ...
    val inputs = _native_attempt_inputs(project, descriptor, operation, _field(fields, "inputs", label))
    val diagnostics = _field(fields, "diagnostics", label).asArray.getOrElse(_invalid("Document Project retained v2 attempt diagnostics must be an array")).map { value =>
      _nonempty(value.asString.getOrElse(_invalid("Document Project retained v2 attempt diagnostics must contain only strings")), "Document Project retained v2 attempt diagnostic")
    }.toVector
    if (diagnostics.isEmpty)
      _invalid("Document Project retained v2 attempt diagnostics must be non-empty")
    val outcome = _string(fields, "outcome", label)
    val outputvalues = _field(fields, "outputs", label).asArray.getOrElse(_invalid("Document Project retained v2 attempt outputs must be an array"))
    val outputs = outcome match {
      case "accepted" =>
        ...
        val parsed = outputvalues.zip(declaration.outputs).map {
          case (value, declared) => _native_attempt_output(project, value, declared)
        }.toVector
        val receiptfields = _object(_field(fields, "receipt", label), "Document Project retained accepted v2 attempt receipt")
        ...
        val receiptidentity = _nonempty(_string(receiptfields, "identity", "Document Project retained accepted v2 attempt receipt"), "Document Project retained accepted v2 attempt receipt identity")
        val receiptvalue = _nonempty(_string(receiptfields, "value", "Document Project retained accepted v2 attempt receipt"), "Document Project retained accepted v2 attempt receipt value")
    ...
    Attempt(FileIdentity(CozyDocumentProject._project_relative(project, path), _sha256(path)), _attempt_v2_schema, operation.id, outcome, operation.produces, inputs, outputs)
  }
  ...
  private def _native_attempt_inputs(
    ...
    inputvalues.zip(expected).map {
      case (inputvalue, expectedpath) =>
  ...
  private def _native_attempt_output(
    ...
    val identity = _nonempty(_string(fields, "identity", "Document Project retained accepted v2 attempt output"), "Document Project retained accepted v2 attempt output identity")
    val path = _string(fields, "path", "Document Project retained accepted v2 attempt output")
    val mediatype = _nonempty(_string(fields, "mediaType", "Document Project retained accepted v2 attempt output"), "Document Project retained accepted v2 attempt output mediaType")
  ...
  private def _native_input_paths(
    ...
    if (operation.id == "article.render-review")
      ...
      _invalid("native direct input identities are unsupported for this operation")
  ...
  private def _identity_current(project: Path, identity: FileIdentity): Boolean = {
    ...
    if (!path.startsWith(project)) {
      ...
      directancestors && !Files.isSymbolicLink(path) && Files.isRegularFile(path, LinkOption.NOFOLLOW_LINKS) && _sha256(path) == identity.sha256
    }
  }
  ...
  private def _retained_file_identity(value: Json, label: String): FileIdentity = {
    ...
    _retained_identity(path, sha256, label)
  }

  private def _retained_identity(path: String, sha256: String, label: String): FileIdentity = {
    if (!_relative_path(path) || !_hash_pattern.pattern.matcher(sha256).matches())
      _invalid(s"$label identity is invalid")
