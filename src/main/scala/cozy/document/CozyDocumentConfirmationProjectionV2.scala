...
private[cozy] object CozyDocumentConfirmationProjectionV2 {
  ...
  private def _page(validated: CozyDocumentDescriptionV2.ValidatedDocument, vocabulary: Vocabulary, outputidentity: String): String = {
    ...
    val prose = description.document.sections.map(section => _section(section, selected, vocabulary, labels, 2)).mkString
  ...
  private def _structure_relation(relation: CozyDocumentLogicTree.Relation, structure: CozyDocumentLogicTree.Structure, nodelabels: Map[String, String], vocabulary: Vocabulary): String = {
    ...
    s"""<div class="relation" data-relation-id="${_html(relation.id)}" data-relation-type="${_html(relation.relationType)}"><div class="concept" data-node-id="${_html(from.id)}" data-node-role="${_html(from.role)}">${_html(nodelabels(from.id))}</div><div class="arrow"><small>${_html(vocabulary.relationTypes(relation.relationType))}</small><span class="edge-mark" aria-hidden="true">→</span></div><div class="concept" data-node-id="${_html(to.id)}" data-node-role="${_html(to.role)}">${_html(nodelabels(to.id))}</div></div>"""
  }

  private def _section(section: CozyDocumentDescriptionV2.Section, selected: Selection, vocabulary: Vocabulary, labels: Map[String, String], level: Int): String = {
    val heading = math.min(level, 6)
    val blocks = section.blocks.map(block => _block(block, selected, vocabulary, labels)).mkString
    val children = section.sections.map(child => _section(child, selected, vocabulary, labels, level + 1)).mkString
  ...
  private def _block(block: CozyDocumentDescriptionV2.Block, selected: Selection, vocabulary: Vocabulary, labels: Map[String, String]): String = block match {
    case value: CozyDocumentDescriptionV2.Paragraph =>
      ...
      s"""<div class="document-block${_highlight_class(value.coreRefs, selected)}" $target><p>${_html(value.text)}</p>${_reference_chips(value.coreRefs)}</div>"""
    case value: CozyDocumentDescriptionV2.ListBlock =>
      val target = _target_attributes(value.coreRefs, selected, s"""data-block-id="${_html(value.id)}"""")
      val items = value.items.map(item => _list_item(item, selected)).mkString
      s"""<div class="document-block document-list${_highlight_class(value.coreRefs, selected)}" $target><ul>$items</ul>${_reference_chips(value.coreRefs)}</div>"""
    case value: CozyDocumentDescriptionV2.Example =>
      val target = _target_attributes(value.coreRefs, selected, s"""data-block-id="${_html(value.id)}"""")
      s"""<article class="document-block document-example${_highlight_class(value.coreRefs, selected)}" $target><h3>${_html(value.title)}</h3><p>${_html(value.text)}</p>${_reference_chips(value.coreRefs)}</article>"""
    case value: CozyDocumentDescriptionV2.Note =>
      val target = _target_attributes(value.coreRefs, selected, s"""data-block-id="${_html(value.id)}"""")
      s"""<aside class="document-block document-note${_highlight_class(value.coreRefs, selected)}" $target><h3>${_html(value.title)}</h3><p>${_html(value.text)}</p>${_reference_chips(value.coreRefs)}</aside>"""
    case value: CozyDocumentDescriptionV2.LogicalStructure =>
      ...
      s"""<details class="document-block logical-structure-reference${_highlight_class(refs, selected)}" $target data-step-ref="${_html(value.stepRef)}"><summary>${_html(vocabulary.chrome.logicalStructureReference)}: ${_html(label)}</summary><p class="identity" data-step-ref="${_html(value.stepRef)}">${_html(value.stepRef)}</p></details>"""
  }
  ...
  private def _reference_chips(refs: CozyDocumentDescriptionV2.References): String = {
    val values = refs.steps.map(id => s"""<code>step/${_html(id)}</code>""") ++ refs.relations.map(id => s"""<code>relation/${_html(id)}</code>""")
