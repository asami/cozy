...
private[cozy] object CozyDocumentLogicTreeProjection {
  ...
  private def _overview_html(validated: CozyDocumentLogicTree.Validated): String = {
    val content =
      s"""<main id="logic-tree-overview" data-core-id="${_html(validated.core.id)}" data-core-identity="${_html(validated.coreIdentity)}">
         |<header class="document-heading"><h1>${_html(validated.titlesById(validated.core.root.id))}</h1><p>Logic Tree overview · ${_html(validated.format.locale)}</p></header>
         |${_overview_step(validated, validated.core.root, Vector.empty)}
         |</main>""".stripMargin
    _page(
      s"Logic Tree Overview — ${validated.titlesById(validated.core.root.id)}",
  ...
  private def _slides_html(validated: CozyDocumentLogicTree.Validated): String = {
    ...
    val content =
      s"""<main id="logic-tree-slides" data-core-id="${_html(validated.core.id)}" data-core-identity="${_html(validated.coreIdentity)}">
         |<header class="slide-deck-heading"><h1>${_html(validated.titlesById(validated.core.root.id))}</h1><p>${validated.depthFirstSteps.size} depth-first Step pages · ${_html(validated.format.locale)}</p></header>
         |$pages
         |</main>
    ...
    _page(
      s"Logic Tree Step Slides — ${validated.titlesById(validated.core.root.id)}",
      validated.format.locale,
  ...
  private def _slide_page(
    ...
    val previous = if (index == 0) "<span class=\"navigation-disabled\">Previous</span>" else
      s"""<a class="previous" rel="prev" href="#slide-${_html(validated.depthFirstSteps(index - 1).id)}">Previous</a>"""
    val next = if (index + 1 == count) "<span class=\"navigation-disabled\">Next</span>" else
      s"""<a class="next" rel="next" href="#slide-${_html(validated.depthFirstSteps(index + 1).id)}">Next</a>"""
    s"""<section class="slide-page" id="slide-${_html(step.id)}" tabindex="-1" data-step-id="${_html(step.id)}" data-page-number="${index + 1}">
       ...
       |<nav class="slide-navigation" aria-label="Step page navigation">$previous <a class="overview-link" href="#logic-tree-slides">Deck</a> $next</nav>
       |</section>""".stripMargin
  }
  ...
  private def _claims(validated: CozyDocumentLogicTree.Validated, step: CozyDocumentLogicTree.Step): String = {
    ...
    s"""<section class="step-claims" aria-label="Claims for ${_html(step.id)}"><h3>Claims</h3><ul>$values</ul></section>"""
  }
  ...
  private def _structure(validated: CozyDocumentLogicTree.Validated, step: CozyDocumentLogicTree.Step): String = {
    ...
    s"""<section class="local-structure" id="structure-${_html(step.id)}" data-pattern="${_html(step.structure.pattern)}"><h3>Local Structure · ${_html(step.structure.pattern)}</h3><ol class="logic-nodes">$nodes</ol><ol class="typed-relations">$relations</ol></section>"""
  }

  private def _children(validated: CozyDocumentLogicTree.Validated, step: CozyDocumentLogicTree.Step): String = {
    val values = if (step.steps.isEmpty) "<li>No direct children.</li>" else step.steps.map { child =>
      s"""<li data-child-step-id="${_html(child.id)}"><a href="#slide-${_html(child.id)}">${_html(validated.titlesById(child.id))}</a></li>"""
    }.mkString
    s"""<section class="direct-children" aria-label="Direct children for ${_html(step.id)}"><h3>Direct children</h3><ul>$values</ul></section>"""
  }

  private def _flow(validated: CozyDocumentLogicTree.Validated, step: CozyDocumentLogicTree.Step, targetPrefix: String): String = {
    val values = if (step.flow.transitions.isEmpty) "<li>No direct-child Flow transitions.</li>" else step.flow.transitions.map { transition =>
    ...
    s"""<section class="direct-child-flow" id="flow-${_html(step.id)}" aria-label="Direct-child Flow for ${_html(step.id)}"><h3>Direct-child Flow</h3><ol>$values</ol></section>"""
  }
