...
Cozy has two explicit producer operations. `cozy bok publish-video` remains
the video producer. `cozy bok publish-media <project-dir> [--publication
...
The operation then replaces only its `(articleIdentity, locale, role)`,
