...
## Future Visual Page coexistence
...
The future Visual Page route described in
...
`pptx`, `slides`, and `montage`. The new route receives verified v2 renderer
and review evidence bound to Visual Page Set/page, catalog, binding, template,
renderer, and asset identities only after explicit implementation.

Existing `cozy.media.receipt.v2` and `cozy.media.review-state.v1` schema shapes
do not change. Only the implemented visual-page route may record named page,
catalog, binding, and asset inputs in the existing ordered receipt input
evidence. This does not transfer current receipt or review-state behavior to
the new route, nor does it make a Visual Page, binding, template, or generated
PPTX a semantic authority. Cozy's future responsibility remains validation,
identity, orchestration, and stale-output evidence; the external renderer owns
runtime rendering and external consumers own acceptance. No v2 implementation
