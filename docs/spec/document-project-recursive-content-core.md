...
`stepBindings`, `claimBindings`, and `nodeBindings`. Its schema is exactly
...
wording binding, and the Format has no unknown wording identity.

## Projections

For either admitted input pair, repeated rendering produces equal UTF-8 HTML
and output identity. Overview includes every Step, local Structure, typed
Relation, and direct-child Flow in nested containment order. Slides include
one 16:9 depth-first page for each Step, ancestor context, direct children,
local Structure, Flow, previous/next links, ArrowLeft/ArrowRight navigation,
and a print page boundary for each Step.
...
real Article 9 Japanese fixture and the closed rejection, identity,
