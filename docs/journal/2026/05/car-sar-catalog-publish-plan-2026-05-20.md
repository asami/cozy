...
| CS-08B | Generated sbt scaffold dependency simplification | open | Cozy/simple-modeler build.sbt scaffold templates still emit the old broad dependency list. Generated CAR component projects should use the same minimal direct dependency policy proven by CS-08. | Update cozy `car-sbt-project` and simple-modeler Scala realm build.sbt templates to emit direct `goldenport-cncf` plus ScalaTest only, unless a generator-specific dependency is proven necessary; update scaffold tests. | CS-07, CS-08 |
