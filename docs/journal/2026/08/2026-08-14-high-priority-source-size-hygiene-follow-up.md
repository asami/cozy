...
## Post-closure source-size follow-up

### HYG-SCV-01: Split `CozyScaffold.scala`

- Hygiene Status: RESOLVED
- Discovered: 2026-08-20 during `cncf-validated-commit` focused review.
- Repository and path: `cozy:src/main/scala/cozy/scaffold/CozyScaffold.scala`.
- Historical discovery evidence: the source had 1,848 lines and owned independently changing CAR,
  CAR-SAR, project metadata, README, launcher, and generated-script emitters.
- Category and priority: source-size debt; normal-threshold breach (P2).
- Risk: unrelated scaffold responsibilities remain coupled, making version and
  generated-output changes harder to review and validate independently.
- Outside this commit: this change centralizes version defaults and repairs
  their generated output; a behavior-preserving responsibility split would
  broaden the frozen boundary substantially.
- Resolution evidence: pre-split `CozyScaffold.scala` measured 1,634 lines.
  The responsibility map now keeps `CozyScaffold.scala` as the compatibility
  facade plus configuration/default/path helpers;
  `CozyScaffoldComponentTemplates.scala` owns CAR/CAR-SAR project-template
  rendering; and `CozyScaffoldRuntimeScriptTemplates.scala` owns generated
  runtime and launcher-script rendering.
- Invariants: facade/API call shapes and default arguments remain callable from
  `CozyScaffold`; facade defaults remain retained, while renderer-local immutable
  BuildInfo defaults preserve template output; generated project and script bytes
  are preserved.
