# Phase 41 Checklist: Explanation Structure Review HTML

This checklist is the authoritative progress ledger for Phase 41. It is not a
normative behavior contract.

Phase Status: PLANNED; NOT STARTED

## PREVIEW41-01: Review Contract and Information Architecture

Stage Status:
- Current status: OPEN
- Owner: Cozy Phase 41
- Update rule: Update this block from the checklist state below.

- [ ] Specify the exact input authority, command grammar, output boundary,
      failure contract, and `cozy.explanation-preview.v1` receipt role.
- [ ] Specify the overview, page-card, and page-detail information structure.
- [ ] Keep HTML/CSS/inline-SVG/interaction data outside semantic identities.
- [ ] Define the preview renderer/profile binding for schematic Visual Pattern
      display without adding physical layout vocabulary to Visual Page IR.

## PREVIEW41-02: Validated Integrated Review Model

Stage Status:
- Current status: OPEN
- Owner: Cozy Phase 41
- Update rule: Update this block from the checklist state below.

- [ ] Reuse the accepted Composition, Plan, Projection Map, Visual Page Set,
      and catalog parsers and validators.
- [ ] Join ordered Steps, presentation mappings, and Pages by explicit IDs.
- [ ] Preserve claims, sources, assets, Logical Patterns, nodes, typed
      Relations, Visual Patterns, parameters, and consumed identities.
- [ ] Reject missing/duplicate mappings, unknown Pages, incompatibility,
      unsafe resources, identity mismatch, and stale inputs diagnostically.

## PREVIEW41-03: Self-Contained HTML Projection

Stage Status:
- Current status: OPEN
- Owner: Cozy Phase 41
- Update rule: Update this block from the checklist state below.

- [ ] Generate one self-contained UTF-8 HTML with embedded CSS, optional
      embedded JavaScript, and inline SVG only.
- [ ] Show explanation flow and Step-to-Page mapping without interaction.
- [ ] Show every page's semantic role, Logical Pattern, Relations, Visual
      Pattern, parameters, and available identity/currentness state.
- [ ] Preserve useful static and printable content when JavaScript is disabled.
- [ ] Prove atomic output and byte-identical repeat generation.

## PREVIEW41-04: Receipt, Currentness, and Workflow Integration

Stage Status:
- Current status: OPEN
- Owner: Cozy Phase 41
- Update rule: Update this block from the checklist state below.

- [ ] Bind every semantic input, preview renderer/profile, and generated HTML
      identity in the preview receipt.
- [ ] Reject old output after any bound input changes.
- [ ] Add command help and deterministic success/error reporting.
- [ ] Preserve `cozy media visual-page preview` behavior and schemas unchanged.

## PREVIEW41-05: Representative Driver Acceptance and Closure

Stage Status:
- Current status: OPEN
- Owner: Cozy Phase 41
- Update rule: Update this block from the checklist state below.

- [ ] Cover `problem-solution`, `product-overview`, and one Step-to-multiple-
      Pages projection.
- [ ] Confirm that flow, page mapping, logical semantics, and visual structure
      are understandable together in the default overview.
- [ ] Use a deterministic repository-controlled accepted Visual Page fixture as
      the representative driver and verify page identity/order agreement
      without treating PDF/PPTX as authority. Article 8 runtime acceptance is
      optional post-Phase-42 operational follow-up only.
- [ ] Run focused and full serialized Cozy validation.
- [ ] Complete independent Phase review and synchronize Strategy, Phase, and
      checklist ledgers before closure.

Phase 41 is PLANNED and NOT STARTED. No implementation, validation,
publication, deployment, push, or downstream consumer acceptance is claimed.
